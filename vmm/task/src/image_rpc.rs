...
use containerd_shim::{other, other_error, Error, Result};
use image_rs::{bundle::BUNDLE_ROOTFS, image::ImageClient};
...
impl KuasarImageClient {
    pub fn new(task_config: &TaskConfig) -> Self {
        ...
        if !task_config.image_config.image_policy_file.is_empty() {
            image_client.config.file_paths.policy_path =
                task_config.image_config.image_policy_file.clone();
        }
        if !task_config
            .image_config
            ...
            .is_empty()
        {
            image_client.config.file_paths.sigstore_config = task_config
                .image_config
                .simple_signing_sigstore_config
                .clone();
        }
        ...
        info!(
            "enable_signature_verification set to: {}",
    ...
    async fn common_image_pull(
        ...
        match res {
            Ok(image) => {
                info!(
                    "Successfully pull and unpack image {:?}, cid: {:?}, with image-rs. ",
                    image, cid
                );
            }
            Err(e) => {
                error!(
                    "Failed to pull and unpack image {:?}, cid: {:?}, with image-rs {:?}. ",
                    image,
                    cid,
                    e.to_string()
                );
    ...
    pub async fn pull_image_for_container(&self, image: &str, cid: &str) -> Result<String> {
        ...
        fs::create_dir_all(&bundle_path)
            .map_err(|e| other!("failed to create dir {:?} {:?}", &bundle_path, e))?;
    ...
    pub async fn merge_bundle_oci(&self, container_oci: &mut Spec) -> Result<()> {
        ...
        if let Some(image_name) = annotations.get(ANNO_K8S_IMAGE_NAME) {
            ...
            if let Some(container_id) = images.get(image_name) {
                ...
                let image_oci = Spec::load(image_oci_config_path.to_str().ok_or_else(|| {
                    other!(
                ...
                if let Some(mut container_root) = container_oci.root().clone() {
                    if let Some(image_root) = image_oci.root() {
                        ...
                        container_root.set_path(std::path::PathBuf::from(&String::from(
        ...
        if let Some(mut env) = target.env().clone() {
            for source_env in source.env().clone().unwrap_or_default() {
                ...
                if !target
                    .env()
                    .iter()
                    .any(|i| i.contains(&variable_name[0].to_string()))
                {
